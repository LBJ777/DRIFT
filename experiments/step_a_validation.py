...
def extract_xT_features(xT_by_source: dict) -> tuple:
    ...
    返回：
      features: ndarray (N_total, C*H*W) 展平的 x_T
    ...
    for idx, name in enumerate(label_names):
        ...
        for xT in xTs:
            flat = xT.numpy().flatten()
            features_list.append(flat)
            labels_list.append(idx)

    features = np.stack(features_list, axis=0)   # (N, D)
...
def test_psd(xT_by_source: dict, image_size: int, output_path: str) -> dict:
    ...
    if sd_labels:
        ...
        if std_psd > 1e-12:
            # 取目标频率行的最大值
            target_row_max = sd_psd[target_freq_H, :].max()
            sd_peak_zscore = (target_row_max - mean_psd) / std_psd
        print(f"  SD 来源 [{sd_labels[0]}] 在 f=H/8={target_freq_H} 处 PSD z-score: {sd_peak_zscore:.4f}")
    else:
...
def test_ks_gaussianity(xT_by_source: dict) -> dict:
    """
    对每类来源的 x_T 展平值执行 Kolmogorov-Smirnov 检验（vs 标准正态分布）。

    判定标准：
      - 真实图像（real）的 KS p-value > 0.05（不拒绝高斯假设）
      - 其他来源 KS p-value < 0.05（拒绝高斯假设，说明分布有差异）
    """
    print("\n[Test 4] 执行 KS 高斯性检验...")
    results = {}
    real_labels = [l for l in xT_by_source if l.lower() in ("real", "genuine", "authentic")]
    ...
    for label, xTs in xT_by_source.items():
        ...
        # KS 检验（vs 标准正态）
        # 注意：需要先标准化（KS 检验对尺度敏感），此处直接与标准正态比较
        ks_stat, p_value = ks_2samp(
            all_vals,
            np.random.default_rng(42).standard_normal(min(len(all_vals), 10000)),
        )
        is_real = label in real_labels
        print(f"  [{label}] KS 统计量={ks_stat:.6f}, p-value={p_value:.6e}"
              f"  ({'PASS 高斯' if p_value > 0.05 else 'REJECT 高斯'}) {'<-- 真实图像' if is_real else ''}")

        results[label] = {
            "ks_statistic": float(ks_stat),
            "p_value": float(p_value),
            "is_gaussian": bool(p_value > 0.05),
            "is_real_source": is_real,
        }

    # 判定逻辑：real p > 0.05，其他 p < 0.05
    passed = True
    has_real = bool(real_labels)
    if not has_real:
        # 如果没有标注 real 来源，无法评估该项，标记为 N/A
        passed = False
        note = "未找到名为 'real' 的来源目录，无法评估高斯性假设。"
    else:
        for label, res in results.items():
            if label in real_labels:
                if not res["is_gaussian"]:
                    passed = False  # real 应该通过高斯检验
            else:
                if res["is_gaussian"]:
                    # 非 real 来源通过了高斯检验，假设减弱（警告但不强制 fail）
                    warnings.warn(f"来源 [{label}] 通过了高斯检验（p={res['p_value']:.4f}），"
                                  "可能与真实图像分布相似。")
        note = ("real 来源 p-value > 0.05（高斯），其他来源 p-value < 0.05（非高斯）"
                if passed else "real 来源高斯性检验未通过。")

    return {
        "per_source": results,
        "passed": passed,
        "note": note,
        "threshold_real_p": 0.05,
    }
